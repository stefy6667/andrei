...
def test_twilio_initial_prompt_falls_back_to_say_when_tts_provider_errors():
    from app.config import settings
    ...
    async def failing_synthesize(text: str, language: str):
        raise RuntimeError("simulated tts provider failure")

    settings.elevenlabs_api_key = "test-key"
    settings.tts_provider_ro = "elevenlabs"
    telephony.tts_client.synthesize = failing_synthesize
    try:
        res = client.post("/twilio/voice", data={"CallSid": "CA-EL-ERR", "SpeechResult": ""})
        assert res.status_code == 200
